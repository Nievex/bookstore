...
function doInsert(){
	...
	function processorder() {
		...
		$customer_id = $_SESSION['CUSID']; // Assuming this is how you're getting the customer ID
	
		// Verify the customer ID exists
		$mydb->setQuery("SELECT CUSTOMERID FROM tblcustomer WHERE CUSTOMERID = '{$customer_id}'");
		...
		if (!isset($_POST['alltot'])) {
			// Handle the case when the 'alltot' field is not present in the form data
			message("Total payment amount is missing.", "error");
		...
		// Continue with the order processing
		...
		for ($i = 0; $i < $count_cart; $i++) {
			...
			$order->CUSID = $customer_id; // Ensure this is set correctly
			$order->ORDEREDNUM = $_POST['ORDEREDNUM'];
	
			if (!$order->create()) {
				die("Failed to create order: " . $mydb->error_msg);
			}
		...
		$summary->CUSTOMERID = $customer_id;
		$summary->ORDEREDNUM = $_POST['ORDEREDNUM'];
		// $summary->DELFEE = $_POST["DELFEE"];
